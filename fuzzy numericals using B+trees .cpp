...
// Structure to represent a fuzzy interval
struct Interval {
    int start;
    int end;
    int tolerance; // Tolerance level for fuzzy matching

    Interval(int s, int e, int t) : start(s), end(e), tolerance(t) {}
    
    // Function to check if a point is within the fuzzy interval
...
int main() {
    ...
    // Generate random student marks and use them to insert fuzzy intervals into the tree
    random_device rd;
    mt19937 gen(rd());
    uniform_int_distribution<int> marks(0, 100);
    uniform_int_distribution<int> tolerance(1, 5); // Random tolerance level between 1 and 5
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        cout << "Inserted fuzzy interval [" << min(start, end) << ", " << max(start, end) << "] with tolerance " << t << endl;
    }
